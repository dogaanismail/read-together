...
const Profile = () => {
  ...
  const displayUser = {
    ...
    username: user.username ? `@${user.username}` : `@${user.firstName.toLowerCase()}${user.lastName.toLowerCase()}`,
    bio: user.bio || "Passionate reader who loves sharing stories. Been practicing speech through reading.",
    joinDate: "March 2023", // This could be derived from user creation date if available
    stats: {
      sessions: user.totalSessions || 0,
      followers: 128, // This would come from a separate API
      following: 89, // This would come from a separate API
      totalLikes: 342, // This would come from a separate API
      booksRead: 12, // This could be calculated from reading sessions
      totalHours: Math.round((user.totalReadingTimeSeconds || 0) / 3600)
    },
    languages: ["English"] // This could be derived from user preferences
  };
  ...
  }, [sessionFilters, searchTerm, userSessions]);
      ...
      <div className="max-w-6xl mx-auto px-4 py-8 sm:px-6 lg:px-8">
        ...
        <Card className="p-8 mb-8 dark:bg-gray-800/50 dark:backdrop-blur-sm dark:border-gray-700/50 dark:shadow-2xl">
            ...
            <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6">
              <Avatar className="h-24 w-24">
                {user.profilePictureUrl ? (
                  <AvatarImage src={user.profilePictureUrl} alt={displayUser.name} />
                ) : (
              ...
              <div className="text-center sm:text-left">
                <h1 className="text-3xl font-bold text-gray-900 dark:text-gray-100 mb-2">{displayUser.name}</h1>
                <p className="text-gray-600 dark:text-gray-300 mb-2">{displayUser.username}</p>
                <p className="text-gray-700 dark:text-gray-200 mb-4 max-w-md">{displayUser.bio}</p>
                
                <div className="flex flex-wrap gap-2 justify-center sm:justify-start mb-4">
                  {displayUser.languages.map((lang) => (
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400">Joined {displayUser.joinDate}</p>
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-6 gap-6 mt-8 pt-8 border-t border-gray-200 dark:border-gray-700">
            <div className="text-center">
              <div className="text-2xl font-bold text-gray-900 dark:text-gray-100">{displayUser.stats.sessions}</div>
              <div className="text-gray-600 dark:text-gray-300">Sessions</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-gray-900 dark:text-gray-100">{displayUser.stats.booksRead}</div>
              <div className="text-gray-600 dark:text-gray-300">Books</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-gray-900 dark:text-gray-100">{displayUser.stats.totalHours}h</div>
              <div className="text-gray-600 dark:text-gray-300">Total Time</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-gray-900 dark:text-gray-100">{displayUser.stats.followers}</div>
              <div className="text-gray-600 dark:text-gray-300">Followers</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-gray-900 dark:text-gray-100">{displayUser.stats.following}</div>
              <div className="text-gray-600 dark:text-gray-300">Following</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-gray-900 dark:text-gray-100">{displayUser.stats.totalLikes}</div>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
          ...
          <TabsContent value="activity" className="space-y-6">
            <div>
              ...
              <ReadingActivityGraph userId={user.id} isOwnProfile={true} />
            </div>
          </TabsContent>
